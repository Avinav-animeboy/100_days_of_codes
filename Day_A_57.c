...
int main() {
    int n, i, j, prev;  // declare all variables first
    ...
    for (i = 0; i < n; i++) {
        prev = -1;  // assume no previous greater element yet

        // look to the left side for a greater element
        for (j = i - 1; j >= 0; j--) {
            if (arr[j] > arr[i]) {   // found previous greater
                prev = arr[j];
